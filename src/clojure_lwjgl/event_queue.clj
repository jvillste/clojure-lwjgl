...
(defn add-event-handlers [gui types event-handler]
  (if (first types)
    (recur (add-event-handler gui (first types) event-handler)
           (rest types)
           event-handler)
    gui))
  ...
  (assoc gui
    :event-handlers (let [event-handler-vector ((:event-handlers gui) type)]
                      (if (= 1 (count event-handler-vector))
                        ...
                        (assoc-in (:event-handlers gui) [type] (vec (filter (fn [event-handler-candidate]
